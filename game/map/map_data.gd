...
func setup_pathfinding() -> void:
	...
	pathfinder.region = Rect2i(0, 0, width, height) ## Create pathfinding grid from corner to corner
	pathfinder.update()
	for y in height:
		for x in width:
			var grid_position := Vector2i(x, y)
			var tile: Tile = get_tile(grid_position)
			pathfinder.set_point_solid(grid_position, not tile.is_walkable()) ## Inside of our grid, set every point that is not walkable as not walkable
